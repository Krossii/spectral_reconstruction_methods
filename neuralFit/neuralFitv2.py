...
def KL_kernel_Position_Vacuum_Symm(Position, Omega, T):
    Position = np.asarray(Position)[:, None]   # (Nτ, 1)
    Omega    = np.asarray(Omega)[None, :]      # (1, Nω)
    beta = 1.0 / T
    return np.exp(-Omega * np.abs(Position)) + np.exp(-Omega * (beta - np.abs(Position)))
...
def KL_kernel_Omega(KL, finiteT_kernel: bool, x, Omega, args=()):
    ret = KL(x, Omega, *args)
    ret[:, Omega == 0] = 1
    ret = Omega * ret
    ...
    if finiteT_kernel:
        if len(args) < 1:
            raise ValueError("finiteT_kernel=True requires args[0] for the Omega->0 limit.")
...
class networkParameters:
    ...
    saveWeightHistograms: bool=False
...
class neuralFit:
    def __init__(self,networkParameters:networkParameters):
        ...
        self.saveWeightHistograms=networkParameters.saveWeightHistograms
    ...
    def initKernel(self,extractedQuantity:str,finiteT_kernel:bool,Nt:int,x:np.ndarray,omega:np.ndarray):
        if extractedQuantity=="RhoOverOmega" and finiteT_kernel:
            kernel=KL_kernel_Omega(KL_kernel_Position_FiniteT,finiteT_kernel,x,omega,args=(1/Nt,))
        elif extractedQuantity=="RhoOverOmega" and finiteT_kernel==False:
            kernel=KL_kernel_Omega(KL_kernel_Position_Vacuum_Symm,finiteT_kernel,x,omega,args=(1/Nt,))
            ...
            kernel=KL_kernel_Position_Vacuum_Symm(x,omega,1/Nt)
        else:
    ...
    def fitCorrelator(self, x: np.ndarray, error: np.ndarray, correlator: np.ndarray, finiteT_kernel: bool, Nt: int, omega: np.ndarray, extractedQuantity: str = "RhoOverOmega", verbose: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if getattr(self, "saveWeightHistograms", False):
            os.makedirs("weight_histograms", exist_ok=True)
            os.makedirs("weight_data", exist_ok=True)
            os.makedirs("activation_stats", exist_ok=True)
            ...
            for i, layer in enumerate(model.layers):
                ...
                if weights:  # Some layers (like Dropout) have no weights
                    for j, weight_matrix in enumerate(weights):
                        ...
                        # Plot histogram
                        plt.figure()
                        plt.hist(flat_weights, bins=100, log=True)
                        plt.title(f"Layer {i} - Weight {j}")
                        plt.xlabel("Weight Value")
                        plt.ylabel("Frequency (log scale)")
                        plt.grid(True)
                        plt.tight_layout()
                        plt.savefig(f"weight_histograms/layer_{i}_weight_{j}.png")
                        plt.close()

                        # Save weight array
                        np.save(f"weight_data/layer_{i}_weight_{j}.npy", weight_matrix)

                        # Save sparsity stats
                        stats = {
                            "num_weights": len(flat_weights),
                            "num_zeros": int(np.sum(flat_weights == 0)),
                            "num_near_zero": int(np.sum(np.abs(flat_weights) < 1e-5)),
                        ...
                        with open(f"weight_data/layer_{i}_weight_{j}_stats.txt", "w") as f:
            ...
            for i, act in enumerate(activations):
                flat_act = tf.reshape(act, [-1]).numpy()
                stats = {
                    "num_activations": len(flat_act),
                    "num_zeros": int(np.sum(flat_act == 0)),
                    "num_near_zero": int(np.sum(np.abs(flat_act) < 1e-5)),
                    "sparsity_ratio_near_zero": float(np.mean(np.abs(flat_act) < 1e-5)),
                    "min": float(np.min(flat_act)),
                    "max": float(np.max(flat_act)),
                    "mean": float(np.mean(flat_act)),
                    "std": float(np.std(flat_act)),
                }

                with open(f"activation_stats/layer_{i}_activation_stats.txt", "w") as f:
                    for k, v in stats.items():
                        f.write(f"{k}: {v}\n")

                np.save(f"activation_stats/layer_{i}_activations.npy", act)
...
class ParameterHandler:
    ...
    def getNetworkParams(self) -> networkParameters:
        return networkParameters(
            ...
            saveWeightHistograms=self.params["saveWeightHistograms"]
        )
